function POSTAjax(n, t, i, r, u, f) {
    $.ajax({
        "async": f, url: n, data: t, type: "POST", cache: !1, beforeSend: function () {
            i()
        }, success: function (n) {
            r(n)
        }, error: function () {
...
function empty(n) {
    ...
    if (typeof n == "object") {
        for (t in n) if (typeof n[t] != "function") return !1;
        return !0
    }
    return !1
}
...
function getScriptChatTgdd() {
    location.href.indexOf("www.thegioididong.com") !== -1 && setTimeout(function () {
        var n, t;
        ...
        })) : (t = "https://cdn.thegioididong.com/dmxchat/chatclienttgddmobile.js", $.getScript(t).done(function () {
            console.log("GET SCRIPT CHAT: DONE")
        }))))
    }, 1e4)
}
...
function checkCmtParam() {
    setTimeout(function () {
        ...
        n > 0 && $("#comment").length > 0 ? ($("html, body").animate({scrollTop: $("#comment").offset().top}, 500), setTimeout(function () {
            ...
            setTimeout(function () {
                ...
                var n = window.location.origin + window.location.pathname,
                    t = "<a class='seeAllCmt' href='" + n + "'>Xem táº¥t cáº£ bĂ¬nh luáº­n<\/a>";
                $(".wrap_comment .listcomment").after(t);
                $("#txtEditorExt").remove();
                $(".wrap_seasort").hide();
                $(".wrap_seasort").removeClass("hide")
            }, 1e3)
        }, 1e3)) : n == 0 && $("html, body").animate({scrollTop: $("#comment").offset().top}, 500)
    }, 2e3)
}
...
function ShowSearchResult() {
    FL_SHOWMOREPRODUCTRESULT && (FL_SHOWMOREPRODUCTRESULT = !1, showLoadingSrh(), searchQuery.TotalNoselling = $(".listsearch li.top20ProductFilter").data("prodcut"), $.ajax({
        url: "/aj/SearchV2/Product", type: "POST", data: searchQuery, success: function (n) {
            var t;
            FL_SHOWMOREPRODUCTRESULT = !0;
            n == null || n == "" ? (console.log("Opps, something went wrong! Try it later.."), location.reload()) : (t = "<div>" + n + "<\/div>", searchQuery.PageIndex == 0 ? $("ul.listsearch").html($(t).find("ul.listsearch li")) : ($("li.leftRecordFake.hide").remove(), $("ul.listsearch").append($(t).find("ul.listsearch li")), $("li.leftRecordFake.hide").html($(t).find("ul.listsearch li.leftRecordFake.hide"))), $(t).find("a.viewmore").length > 0 ? $(".left_search a.viewmore").html() != undefined ? $(".left_search a.viewmore").html($(t).find("a.viewmore").html()) : $("ul.listsearch").after($(t).find("a.viewmore")) : $(".left_search a.viewmore").remove(), LazyLoad());
            var i = $("div.searchCategoryResult a.active").data("total"),
                r = $(".listsearch li.top20ProductFilter").data("prodcut"),
                u = i + " káº¿t quáº£ tĂ¬m tháº¥y Ä‘á»u lĂ  sáº£n pháº©m khĂ´ng kinh doanh / táº¡m háº¿t hĂ ng";
            searchQuery.PageIndex == 0 ? r == 0 ? (FakeCountTotal(), ShowResultListTopProductSelling(u)) : ($("p.text-noselling").addClass("hide"), $("ul.filter").show(), $(".topselling").length > 0 && $(".topselling").remove(), $("p.title-20Product").length > 0 && $("p.title-20Product").remove(), hideLoadingSrh(), LazyLoad()) : (FakeCountTotal(), hideLoadingSrh())
        }
    }), LazyLoad())
}

function ShowMoreProductResult() {
    var n = $(".listsearch li.hide.noSelling").length, i = isMobile ? 12 : 15, t;
    if (n > 0 && n <= i) return $(".listsearch li.hide.noSelling").removeClass("hide"), n = $(".listsearch li.hide.noSelling").length, t = $(".listsearch li.leftRecordFake").data("total"), t == n ? $("a.viewmore").remove() : $("a.viewmore").html("CĂ²n " + (t - n) + " sáº£n pháº©m <span><\/span>"), !1;
    searchQuery.PageIndex = productResultPageIndex++;
    ShowSearchResult()
}

function showLoadingSrh() {
    $(".loadingcover").fadeIn()
}

function hideLoadingSrh() {
    $(".loadingcover").fadeOut()
}

function FilterProp() {
    flagFilterProp || (flagFilterProp = !0, showLoadingSrh(), $("ul.filter").find("li:not(li.sort)").remove(), $.ajax({
        url: "/aj/SearchV2/FilterProp",
        type: "POST",
        data: searchQuery,
        success: function (n) {
            flagFilterProp = !1;
            n == null || n == "" ? console.log("Opps, something went wrong! Try it later..") : $("ul.filter li.sort").before(n)
        }
    }))
}

function ShowResultListTopProductSelling(n) {
    var i = searchQuery.CateID, r = GetQuerystring("clearcache"), t = !1;
    r == 1 && (t = !0);
    $.ajax({
        url: "/aj/SearchV2/TopProductSelling",
        type: "POST",
        data: {cateID: i, clearcache: t},
        success: function (t) {
            if (t == null || t == "") return console.log("Data is null"), $("p.text-noselling").html(n), $("p.text-noselling.hide").removeClass("hide"), $("ul.filter").hide(), $("p.title-20Product").length > 0 && $("p.title-20Product").remove(), $(".topselling").length > 0 && $(".topselling").remove(), hideLoadingSrh(), !1;
            $("p.text-noselling").html(n);
            $("p.text-noselling.hide").removeClass("hide");
            $("ul.filter").hide();
            $(".topselling").length == 0 ? ($(".title-20Product").length > 0 ? $(".title-20Product").after(t) : $(".left_search a.viewmore").length > 0 ? $(".left_search a.viewmore").after(t) : $("ul.listsearch").after(t), CreateSlider()) : ($("p.title-20Product").length > 0 && $("p.title-20Product").remove(), $(".topselling").remove(), $(".title-20Product").length > 0 ? $(".title-20Product").after(t) : $(".left_search a.viewmore").length > 0 ? $(".left_search a.viewmore").after(t) : $("ul.listsearch").after(t), CreateSlider());
            hideLoadingSrh();
            LazyLoad()
        }
    })
...
function GetQuerystring(n, t) {
    t == null && (t = "");
    n = n.replace(/[\[]/, "\\[").replace(/[\]]/, "\\]");
    var r = new RegExp("[\\?&]" + n + "=([^&#]*)"), i = r.exec(window.location.href);
    return i == null ? t : i[1]
}

function FakeCountTotal() {
    var n, t;
    $("ul.listsearch li").length > 0 && (n = $("ul.listsearch li.hide.noSelling").length, n > 0 && (t = $(".listsearch li.leftRecordFake").data("total"), $("a.viewmore").length > 0 ? $("a.viewmore").html("CĂ²n " + (t + n) + " sáº£n pháº©m <span><\/span>") : $("ul.listsearch").after('<a href="javascript:ShowMoreProductResult()" class="viewmore"> CĂ²n ' + n + " sáº£n pháº©m <span><\/span><\/a >")))
}

var lastSuggest, timmer, gl_fLoadChat, productResultPageIndex, FL_SHOWMOREPRODUCTRESULT, flagFilterProp;
(function (n, t) {
    function gt(n) {
    ...
    function lu(n, t, r) {
        var e = !0, u = "width" === t ? n.offsetWidth : n.offsetHeight, f = v(n),
    ...
    var et, bi, o = typeof t, hf = n.location, r = n.document, ki = r.documentElement, cf = n.jQuery, lf = n.$, ot = {},
        b = [], bt = "", di = b.concat, kt = b.push, l = b.slice, gi = b.indexOf, af = ot.toString,
        k = ot.hasOwnProperty, dt = bt.trim, i = function (n, t) {
            return new i.fn.init(n, t, bi)
        }, st = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source, s = /\S+/g, vf = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,
        yf = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/, nr = /^<(\w+)\s*\/?>(?:<\/\1>|)$/, pf = /^[\],:{}\s]*$/,
        wf = /(?:^|:|,)(?:\s*\[)+/g, bf = /\\(?:["\\\/bfnrt]|u[\da-fA-F]{4})/g,
        kf = /"[^"\\\r\n]*"|true|false|null|-?(?:\d+\.|)\d+(?:[eE][+-]?\d+|)/g, df = /^-ms-/, gf = /-([\da-z])/gi,
        ne = function (n, t) {
    ...
    i.extend({
        expando: "jQuery" + (bt + Math.random()).replace(/\D/g, ""), noConflict: function (t) {
            ...
            return n
        }, trim: dt && !dt.call("ï»¿Â ") ? function (n) {
    ...
    bi = i(r), function (n, t) {
        ...
        function oi(n, t, i, r, u, f) {
            return r && !r[o] && (r = oi(r)), u && !u[o] && (u = oi(u, f)), c(function (f, e, o, s) {
                var l, c, a, p = [], y = [], w = e.length, k = f || yr(t || "*", o.nodeType ? [o] : o, []),
                    v = !n || !f && t ? k : bt(k, p, n, o, s), h = i ? u || (f ? n : w || r) ? [] : e : v;
        ...
        function vr(n, t) {
            var f = 0, i = t.length > 0, e = n.length > 0, o = function (o, h, c, l, a) {
                var y, g, k, w = [], d = 0, v = "0", nt = o && [], tt = null != a, it = lt,
        ...
        var ut, e, ht, r, ct, hi, kt, lt, g, nt, s, a, v, h, tt, at, ot, o = "sizzle" + -new Date, y = n.document,
            p = 0, di = 0, ci = ri(), li = ri(), ai = ri(), ft = !1, dt = function (n, t) {
                return n === t ? (ft = !0, 0) : 0
            }, st = typeof t, vi = -2147483648, gi = {}.hasOwnProperty, w = [], nr = w.pop, tr = w.push, b = w.push,
            yi = w.slice, it = w.indexOf || function (n) {
            ...
            f = "[\\x20\\t\\r\\n\\f]", et = "(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+", pi = et.replace("w", "w#"),
            wi = "\\[" + f + "*(" + et + ")" + f + "*(?:([*^$|!~]?=)" + f + "*(?:(['\"])((?:\\\\.|[^\\\\])*?)\\3|(" + pi + ")|)|)" + f + "*\\]",
            ni = ":(" + et + ")(?:\\(((['\"])((?:\\\\.|[^\\\\])*?)\\3|((?:\\\\.|[^\\\\()[\\]]|" + wi.replace(3, 8) + ")*)|.*)\\)|)",
            vt = RegExp("^" + f + "+|((?:^|[^\\\\])(?:\\\\.)*)" + f + "+$", "g"), ir = RegExp("^" + f + "*," + f + "*"),
            rr = RegExp("^" + f + "*([>+~]|" + f + ")" + f + "*"), ti = RegExp(f + "*[+~]"),
            ur = RegExp("=" + f + "*([^\\]'\"]*)" + f + "*\\]", "g"), fr = RegExp(ni), er = RegExp("^" + pi + "$"),
            yt = {
                ID: RegExp("^#(" + et + ")"),
                CLASS: RegExp("^\\.(" + et + ")"),
            ...
            }, ii = /^[^{]+\{\s*\[native \w/, or = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/,
            sr = /^(?:input|select|textarea|button)$/i, hr = /^h\d$/i, cr = /'|\\/g,
            k = RegExp("\\\\([\\da-f]{1,6}" + f + "?|(" + f + ")|.)", "ig"), d = function (n, t, i) {
                var r = "0x" + t - 65536;
            ...
            filter: {
                TAG: function (n) {
                    ...
                    return 1 === r && 0 === u ? function (n) {
                        ...
                        var a, k, c, l, v, w, b = s !== e ? "nextSibling" : "previousSibling", y = t.parentNode,
                            g = f && t.nodeName.toLowerCase(), d = !h && !f;
                        if (y) {
                            if (s) {
                                while (b) {
    ...
    i.support = function (t) {
        ...
        return t.ownLast = "0" !== s, i(function () {
            var h, e, f,
                c = "padding:0;margin:0;border:0;display:block;box-sizing:content-box;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;",
    ...
    var d, or, ii = /[\t\r\n\f]/g, ie = /\r/g, re = /^(?:input|select|textarea|button|object)$/i, ue = /^(?:a|area)$/i,
        ri = /^(?:checked|selected)$/i, a = i.support.getSetAttribute, ht = i.support.input;
    i.fn.extend({
        attr: function (n, t) {
    ...
    var ui = /^(?:input|select|textarea)$/i, fe = /^key/, ee = /^(?:mouse|contextmenu)|click/,
        sr = /^(?:focusinfocus|focusoutblur)$/, hr = /^([^.]*)(?:\.(.+)|)$/;
    i.event = {
        global: {},
        ...
        trigger: function (u, f, e, o) {
            var a, v, s, w, l, c, b, p = [e || r], h = k.call(u, "type") ? u.type : u,
        ...
        dispatch: function (n) {
            n = i.event.fix(n);
            var o, e, r, u, s, h = [], c = l.call(arguments), a = (i._data(this, "events") || {})[n.type] || [],
    ...
    var oe = /^.[^:#\[\.,]*$/, se = /^(?:parents|prev(?:Until|All))/, lr = i.expr.match.needsContext,
    ...
    var yr = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video",
        ce = / jQuery\d+="(?:null|\d+)"/g, pr = RegExp("<(?:" + yr + ")[\\s/>]", "i"), ei = /^\s+/,
        wr = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi, br = /<([\w:]+)/,
        kr = /<tbody/i, le = /<|&#?\w+;/, ae = /<(?:script|style|link)/i, oi = /^(?:checkbox|radio)$/i,
        ve = /checked\s*(?:[^=]|=\s*.checked.)/i, dr = /^$|\/(?:java|ecma)script/i, ye = /^true\/(.*)/,
        pe = /^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)>\s*$/g, e = {
    ...
    var rt, v, y, ci = /alpha\([^)]*\)/i, de = /opacity\s*=\s*([^)]*)/, ge = /^(top|right|bottom|left)$/,
        no = /^(none|table(?!-c[ea]).+)/, ru = /^margin/, to = RegExp("^(" + st + ")(.*)$", "i"),
        lt = RegExp("^(" + st + ")(?!px)[a-z%]+$", "i"), io = RegExp("^([+-])=(" + st + ")", "i"), uu = {BODY: "block"},
        ro = {position: "absolute", visibility: "hidden", display: "block"}, fu = {letterSpacing: 0, fontWeight: 400},
    ...
    var uo = /%20/g, fo = /\[\]$/, yu = /\r?\n/g, eo = /^(?:submit|button|image|reset|file)$/i,
    ...
    var w, c, ai = i.now(), vi = /\?/, so = /#.*$/, pu = /([?&])_=[^&]*/, ho = /^(.*?):[ \t]*([^\r\n]*)\r?$/gm,
        co = /^(?:GET|HEAD)$/, lo = /^\/\//, wu = /^([\w.+-]+:)(?:\/\/([^\/?#:]*)(?::(\d+)|)|)/, bu = i.fn.load,
        ku = {}, yi = {}, du = "*/".concat("*");
    try {
        c = hf.href
    } catch (go) {
    ...
    i.extend({
        ...
        ajax: function (n, r) {
            ...
            r = r || {};
            var l, a, e, d, g, v, y, p, u = i.ajaxSetup({}, r), h = u.context || u,
                nt = u.context && (h.nodeType || h.jquery) ? i(h) : i.event, tt = i.Deferred(),
                it = i.Callbacks("once memory"), b = u.statusCode || {}, rt = {}, ut = {}, o = 0, ft = "canceled", f = {
    ...
    i.ajaxPrefilter("json jsonp", function (r, u, f) {
        var e, s, o,
            h = r.jsonp !== !1 && (at.test(r.url) ? "url" : "string" == typeof r.data && !(r.contentType || "").indexOf("application/x-www-form-urlencoded") && at.test(r.data) && "data");
    ...
    var it, yt, po = /^(?:toggle|show|hide)$/, uf = RegExp("^(?:([+-])=|)(" + st + ")([a-z%]*)$", "i"),
        wo = /queueHooks$/, pt = [ko], ft = {
            "*": [function (n, t) {
                var f = this.createTween(n, t), s = f.cur(), r = uf.exec(t), e = r && r[3] || (i.cssNumber[n] ? "" : "px"),
                    u = (i.cssNumber[n] || "px" !== e && +s) && uf.exec(i.css(f.elem, n)), o = 1, h = 20;
                if (u && u[3] !== e) {
                    e = e || u[3];
                    r = r || [];
                    u = +s || 1;
    ...
    i.offset = {
        setOffset: function (n, t, r) {
            ...
            var e = i(n), o = e.offset(), l = i.css(n, "top"), a = i.css(n, "left"),
                v = ("absolute" === f || "fixed" === f) && i.inArray("auto", [l, a]) > -1, u = {}, s = {}, h, c;
...
"function" != typeof Object.create && (Object.create = function (n) {
    ...
    var r = {
        init: function (t, i) {
            ...
            function r(n) {
                var i, r = "";
                if ("function" == typeof t.options.jsonSuccess) t.options.jsonSuccess.apply(this, [n]); else {
            ...
            var n = this;
            if (!1 === n.$elem.is(":visible")) n.$elem.css({opacity: 0}), t.clearInterval(n.autoPlayInterval), t.clearInterval(n.checkVisible); else return !1;
            ...
            var n = this.$elem.hasClass(this.options.baseClass), t = this.$elem.hasClass(this.options.theme);
            ...
            if (t = n(this.options.responsiveBaseWidth).width(), t > (this.options.itemsDesktop[0] || this.orignalItems) && (this.options.items = this.orignalItems), !1 !== this.options.itemsCustom) for (this.options.itemsCustom.sort(function (n, t) {
                return n[0] - t[0]
            }), i = 0; i < this.options.itemsCustom.length; i += 1) this.options.itemsCustom[i][0] <= t && (this.options.items = this.options.itemsCustom[i][1]); else t <= this.options.itemsDesktop[0] && !1 !== this.options.itemsDesktop && (this.options.items = this.options.itemsDesktop[1]), t <= this.options.itemsDesktopSmall[0] && !1 !== this.options.itemsDesktopSmall && (this.options.items = this.options.itemsDesktopSmall[1]), t <= this.options.itemsTablet[0] && !1 !== this.options.itemsTablet && (this.options.items = this.options.itemsTablet[1]), t <= this.options.itemsTabletSmall[0] && !1 !== this.options.itemsTabletSmall && (this.options.items = this.options.itemsTabletSmall[1]), t <= this.options.itemsMobile[0] && !1 !== this.options.itemsMobile && (this.options.items = this.options.itemsMobile[1]);
            ...
            var t = this, i = 0, r = t.itemsAmount - t.options.items;
            ...
            var t = this, i = n('<div class="owl-buttons"/>');
            ...
            for (this.paginationWrapper.html(""), r = 0, u = this.itemsAmount - this.itemsAmount % this.options.items, t = 0; t < this.itemsAmount; t += 1) 0 == t % this.options.items && (r += 1, u === t && (f = this.itemsAmount - this.options.items), i = n("<div/>", {"class": "owl-page"}), e = n("<span><\/span>", {
            ...
            if (this.currentItem += !0 === this.options.scrollPerPage ? this.options.items : 1, this.currentItem > this.maximumItem + (!0 === this.options.scrollPerPage ? this.options.items - 1 : 0)) if (!0 === this.options.rewindNav) this.currentItem = 0, n = "rewind"; else return this.currentItem = this.maximumItem, !1;
            this.goTo(this.currentItem, n)
        }, prev: function (n) {
            if (this.isTransition) return !1;
            if (this.currentItem = !0 === this.options.scrollPerPage && 0 < this.currentItem && this.currentItem < this.options.items ? 0 : this.currentItem - (!0 === this.options.scrollPerPage ? this.options.items : 1), 0 > this.currentItem) if (!0 === this.options.rewindNav) this.currentItem = this.maximumItem, n = "rewind"; else return this.currentItem = 0, !1;
            this.goTo(this.currentItem, n)
        }, goTo: function (n, i, r) {
            var u = this;
            ...
            var i = this;
            i.isCssFinish = !1;
            i.$owlWrapper.stop(!0, !0).animate({left: n}, {
                duration: t || i.options.slideSpeed, complete: function () {
                    i.isCssFinish = !0
                }
            })
            ...
            var t = this, i = !0 === t.options.scrollPerPage ? t.pagesInArray : t.positionsInArray, u = t.newPosX,
                r = null;
            ...
            var i = this, e = n(i.$owlItems[i.currentItem]).find("img"), r;
            ...
            var n = this, u = n.outClass, f = n.inClass, t = n.$owlItems.eq(n.currentItem),
...
$(document).ready(function () {
    ...
    $(".listsearch li button").click(function () {
        location.href = $(this).parent("li").find("a").attr("href")
    });
    n = 0;
    $("div.searchCategoryResult a").each(function () {
        n += $(this).width() + 29
    });
    $("div.searchCategoryResult").width() <= n && $("div.searchCategoryResult .expand label").removeClass("hide");
    $(document).on("click", ".criteria", function () {
        $(".criteria").not(this).next().hide();
        $(this).next().slideToggle(200);
        $(".barpage").length > 0 && $("body,html").animate({scrollTop: $(".barpage").position().top}, 600)
    });
    $(document).on("click", ".closefilter", function () {
        $(".filter li div").fadeOut()
    });
    $("div.searchCategoryResult a").click(function () {
        $(this).hasClass("active") || ($("div.searchCategoryResult a.active").removeClass("active"), $(this).addClass("active"), $("ul.filter li.sort label.check").removeClass("check"), $("ul.filter li.sort label[data-id=0]").addClass("check"), $("ul.filter li.sort div").hide(), productResultPageIndex = 1, searchQuery.CateID = $(this).data("id"), searchQuery.Manus = "", searchQuery.Price = -1, searchQuery.Properties = "", searchQuery.PageIndex = 0, searchQuery.OrderBy = 0, FilterProp($(this).data("id")), ShowSearchResult())
    });
    $("div.searchCategoryResult .expand").click(function () {
        $(this).addClass("hide");
        $("div.searchCategoryResult").removeClass("collapse")
    });
    $(document).on("click", ".filter li.manu label", function () {
        $(this).toggleClass("check");
        var n = "";
        $(".filter li.manu label.check").each(function () {
            n += $(this).data("id") + ","
        });
        searchQuery.Manus = n != "" ? n.substring(0, n.length - 1) : "";
        productResultPageIndex = 1;
        searchQuery.PageIndex = 0;
        ShowSearchResult()
    });
    $(document).on("click", ".filter li.price label", function () {
        $(this).hasClass("check") ? $(".filter li.price label").removeClass("check") : ($(".filter li.price label").removeClass("check"), $(this).addClass("check"));
        productResultPageIndex = 1;
        searchQuery.PageIndex = 0;
        searchQuery.Price = $(".filter li.price label.check").data("id");
        ShowSearchResult()
    });
    $(document).on("click", ".filter li.prop label", function () {
        $(this).toggleClass("check");
        var n = "";
        $(".filter li.prop label.check").each(function () {
            n += $(this).data("id") + ","
        });
        searchQuery.Properties = n != "" ? n.substring(0, n.length - 1) : "";
        productResultPageIndex = 1;
        searchQuery.PageIndex = 0;
        ShowSearchResult()
    });
    $(document).on("click", ".filter li.sort label", function () {
        $(this).hasClass("check") || ($(".filter li.sort label").removeClass("check"), $(this).addClass("check"), productResultPageIndex = 1, searchQuery.PageIndex = 0, searchQuery.OrderBy = $(".filter li.sort label.check").data("id"), ShowSearchResult())
    });
    $(document).on("click", ".report_text label", function () {
        $(this).hasClass("choose") || showLoadingSrh()
    });
    var t = $("div.searchCategoryResult a.active").data("total"),
        i = $(".listsearch li.top20ProductFilter").data("prodcut"),
        r = t + " káº¿t quáº£ tĂ¬m tháº¥y Ä‘á»u lĂ  sáº£n pháº©m khĂ´ng kinh doanh / táº¡m háº¿t hĂ ng";
    i == 0 && ($("p.text-noselling").html(r), $("p.text-noselling.hide").removeClass("hide"));
    FakeCountTotal();
    CreateSlider();
    LazyLoad()
});
